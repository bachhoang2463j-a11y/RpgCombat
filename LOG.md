...
## [LOG-208] 2026-09-07 — 战斗标记点 + 导演兜底人格（f190c9d~5c05560 + README f1a3323）

- **变更行为**（RpgCombat 本体特性，六个提交分步落地）：
  1. **LLM 设置持久化简化（`9851889`）**：`loadLLMSettings`/`persistLLMSettings` 移除实测不生效的酒馆助手 character 变量主通道（getVariables/setVariables），改为纯 localStorage（沿用键 `rpg_combat_llm_settings`，已存数据无缝衔接）。用户附带需求——原"先存聊天记录存不了再降级 local"的通道实际直接落在 local，优先级通道删除。
  2. **战斗标记点解析器 `parseBattleMarker`（`f190c9d`）**：正文 AI 一行 ~30 token 标记【战斗标记点丨我方:…丨敌方:…丨场地:…】宣告开战。我方段可选（省略=状态栏存活全员）；敌方段必填（`*N`/`×N` 数量后缀 + `[变种]` 标签，复用 `parseNameTagAndMultiplier`）；场地段写入我方列表键后缀走 FIELD_EFFECTS 管线。容错：全角/半角冒号竖线、空格、漏段；畸形返回 null 不破坏轮询。`findBattleMarker` 取末块（与 Combat_block lastIndexOf 语义一致）。
  3. **标记组装 `assembleBattleFromMarker`（`d06d3e9`）**：敌名 `resolveWorldbookEnemy` 三级匹配（含动态词条）展开、数量复制、变种标签并入属性栏；我方接 `readStatusBarSnapshot` 数值（HP/MP/Armor，同 §10.11 语义）；未命中敌名/我方名记入黄标名单（`renderMarkerMissWarnings` 备战界面展示，不阻塞开战）。组装产物为完整 YAML（正确缩进：列表项 4 空格 + 字段 8 空格，经 jsyaml 本地验证）。
  4. **标记检测接线（`1011f81`）**：`startSTPolling` tick 内 Combat_block 检测之前插入标记检测——**本楼有手写块时手写块优先（现状零修改）**；命中标记即组装→`onCombatDataReceived`→停轮询（一次性轮询天然幂等）。harness test20 因 IAB iframe 不可见时 setInterval 被冻结（AGENTS.md 已知差异），改为函数级断言（直接驱动 tick 核心链路），interval 实时性留真环境回归。
  5. **导演兜底 Tab（`86f6ccd`）**：【💬 对话】弹窗顶部新增 Tab 栏（对话 | 🎬 导演兜底），兜底 Tab 独立 5 预设 + apiUrl/apiKey/model/温度/max_tokens/系统提示词；独立键 `rpg_combat_director_settings` 纯 localStorage；系统提示词默认值 = 《敌方生成规则精简版.md》全文 JSON 转义内嵌常量 `DEFAULT_DIRECTOR_PROMPT`（139 处反引号故不用模板字符串）+「↺ 恢复默认」；保存按钮统一收集两 Tab。
  6. **兜底生成全链路（`5c05560`）**：两条载入路径开战前 `detectDirectorCandidates`（无技能且图鉴未命中）→ confirm 弹窗（启动生成/跳过）→ `callDirectorLLM`（精简请求函数：独立端点/温度/纯系统提示词，一次请求补全全部）→ `validateDirectorEntry` 硬规则校验（标签白名单 44 技能标签 + 17 变种群标签、Aim 100~200、耗蓝总和 ≤ MP、技能数 ≤ 行动次数+1）→ 失败带错误反馈重试 ≤2 次回落跳过（**不开局死锁**）→ `parseEnemyItem` 替换槽位 → 写回（绑定世界书 `createWorldbookEntries` 追加标准词条 strategy.keys/双向禁递归/render debounced + 重载缓存；否则动态图鉴聊天变量 `$rpg_dynamic_bestiary` 并入同一匹配管线）。【📚 世界书管理】弹窗新增「🎬 动态图鉴」区块（预览/删除单条）。
  7. **README V10.12（`f1a3323`）**：版本头 + §10.7 新章（标记协议、伏击语义映射表：敌方伏击→猝不及防/我方设伏→有备而来、AI 标记协议提示词片段、兜底触发/校验/写回全链路说明）。
  8. **harness 扩容**：test18（解析容错矩阵 21 项）、test19（组装：图鉴展开/数量/变种/场地/状态栏我方/黄标，17 项）、test20（接线：标记建局/手写块优先/幂等/老路径回归，11 项）、test21（兜底：校验器四路/生成替换/动态图鉴落库命中/必失败重试回落/端点降级/世界书写回，20 项）。
- **涉及文件**：`index.html`、`integration-test/harness.html`、`README.md`（`9851889`/`f190c9d`/`d06d3e9`/`1011f81`/`86f6ccd`/`5c05560`/`f1a3323`）。
- **经验证**：Node 语法校验通过（每步）；IAB 跑脚手架最终 **246 项断言全部通过**（test1-17 无回归 + test18-21 新增 69 项）；S4 弹窗交互 13 项 IAB 手动验证通过（tab 切换/预设/恢复默认/持久化/老对话 Tab 回归）。施工事故三起均当场修复：①S1 顺带修复 3ce186c 遗留的 3 处 srcLine 错位（上轮提交后未重跑 harness）；②组装 YAML 缩进层级错误（列表项字段未叠加缩进，jsyaml 本地复现定位）；③内嵌正则 `^(自爆:` 未闭合括号 + 模板脚本 `\/` 转义丢失（new Function 校验拦截）。
- **决策原因**：用户 V2 计划全文（AIRP 三件套职责划分——RpgCombat 承担开战初始化链路，副导演只做纯叙事）。计划期澄清三件：①"LLM 聊天记录持久化死代码"实为 character 变量主通道失效（主文件本无聊天记录持久化代码），按用户指示删通道纯 localStorage；②默认提示词换用《敌方生成规则精简版.md》（用户提供，替代原《敌方NPC战斗属性生成指南.md》——后者全树不存在）；③世界书写回 API 经 TavernHelper 开发文档核实为 `createWorldbookEntries`（纯追加，弃原计划的 updateWorldbookWith 与错误字段名 excludeRecursion/preventRecursion，实际为 strategy.keys/recursion.prevent_*）。IAB 定时器冻结属已知环境差异，test20 按验证分流原则改函数级断言。

## [LOG-209] 2026-09-07 — 标记分隔符改花括号 {{}}（c0e8044）

- **变更行为**（实机排障产物，真酒馆 IAB 调试发现两处结构性冲突后按用户方案简化）：
  1. **冲突一：引号美化污染**。状态栏脚本 replaceString 围栏头是纯三反引号（缺 html 标注）→ TH 渲染层不给 custom-html class → 走 markdown 引号美化 → 状态栏 JS 内 709 处双引号被包成 q 标签 → 主脚本语法炸掉停在「加载中」。战斗前端的三反引号html 围栏正常（TH 直渲染 iframe 免美化）。修复：状态栏 replaceString 围栏补 html 标注。
  2. **冲突二：两条正则的替换区互吞**。状态栏正则尾巴 [\s\S]*$ 吞标记后全部内容；楼层顺序为「状态栏标记在前、战斗块在后」时战斗前端整个被吞。受限尾巴版（负向逐字符扫描）虽逻辑正确，但实机渲染管线仍有叠加异常（DOM 膨胀至 263 万、战斗前端内部 HTML 泄漏为正文）。
  3. **用户拍板方案：换分隔符**。战斗标记从【战斗标记点…】改为 {{战斗标记点…}}——花括号与状态栏【状态栏标记点】在正则层零交集，markdown 不动 {{}} 内的引号与括号；战斗 findRegex 简化为「末个 <Combat_block> 块 或 末个 {{战斗标记点}}」，块分支内部不含斜杠转义（避酒馆导入管线吞斜杠——上轮实机确认粘贴 \/ 会丢斜杠）。
  4. harness 22 处标记字面量同步花括号语法，246 项全绿；regex-前端战斗v11.json 的 replaceString/findRegex 同步（六用例回读验证）。
- **涉及文件**：index.html、integration-test/harness.html、README.md（c0e8044）；F:/Download/regex-前端战斗v11.json 同步更新。
- **经验证**：node 语法校验 + harness 246 项全绿 + JSON 回读六用例（仅花括号标记/双块取末/双标记取末/旧【】语法不再匹配/与状态栏标记共存/无目标）。
- **决策原因**：用户实机反馈「iab 渲染直接崩溃」并给出简化方案——「战斗前端的【】改为{{}}，战斗前端只匹配最后的<Combat>块和{{战斗标记点}}，mms 只匹配最后的<Status>块和【状态栏标记点】，之所以匹配最后是防止 llm 在思维链抽风复述格式要求」。方案成立：换分隔符从根上消除两插件正则替换区的交集。
