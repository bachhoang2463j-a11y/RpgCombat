...
## [LOG-069] 2026-08-18 — 世界书系统交互修整：自动绑定角色卡世界书 + 管理面板搜索/激活置顶 + 缺省属性兜底（V6.16 代码先行）

- **变更行为**：
  1. **自动绑定角色卡已绑定的世界书**（修复"不绑定世界书无法世界书载入"的死锁）：新增 `autoBindWorldbook()`（index.html:4412）——**本地已绑定 > 角色卡主世界书 > 聊天文件世界书 > 全局开启世界书（取第一个）**，命中即写入 `worldbookSettings.boundName` 并持久化。接线：启动序列（index.html:9060）与 `initUI`（index.html:4747）构建战局后各调用一次；`startGameFromWorldbook`（index.html:4514）改为 `autoBindWorldbook()` 兜底自动绑定，未检测到任何可用世界书时才提示用户先在酒馆绑定图鉴。
  2. **启动预加载**：`initUI` 中若已绑定世界书且未缓存、未在加载（`window._wbPreloading` 防重入），后台异步 `loadWorldbookData()` 预载词条，使玩家打开【世界书载入】或【世界书管理】时已就绪。
  3. **世界书管理面板新增搜索 + 激活词条置顶**：
     - 面板新增搜索输入框（index.html:4585），`oninput` 更新 `worldbookSearch` 并重渲染；
     - `renderWorldbookEntryList`（index.html:4636）**过滤**：在名称/别名/词条内容关键字/当前战局可命中该词条的敌方名中做不区分大小写匹配；**排序**：激活（启用）词条置顶、禁用词条沉底，保持原相对顺序；
     - **世界书搜索**：搜索词在世界书列表（优先全局开启 `getGlobalWorldbookNames`，否则全部 `getWorldbookNames`）唯一命中时，`worldbookSmartSwitch()`（index.html:4673）自动切换绑定并刷新词条，缩窄搜索范围；面板同时展示全部可用世界书名单（`📚 可用世界书`）供直接识别。
  4. **`parseEnemyItem` 缺省属性兜底**（index.html:4341）：`名字` 外缺 `属性`/`技能` 时赋基础属性串 `[HP:100/100][MP:10/10][Atk:40][Armor:5][Speed:55]`，保证纯名字的敌方项（冲突单列、无数据点名）能解析出可上场实体，载入后仍按名字走图鉴匹配。
  5. **世界书载入按钮配色统一**（index.html:811）：【📚 世界书载入】去掉突兀的紫靛渐变，改为与【🧾 Yaml 载入】一致的灰蓝渐变（`from-gray-800 to-gray-700`…）整体风格。
