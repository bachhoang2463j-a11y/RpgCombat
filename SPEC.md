...
本项目旨在提供一个**轻量级、单文件、零依赖、无后端需求的通用 RPG 回合制战斗引擎前端** (`战斗前端-爬塔 V5.2.html`)。
...
- **`[延迟:N]`**：技能先"蓄力"N 回合（英雄处于 `⏳蓄力中`），**蓄力期间只处于准备态、不结算任何技能效果**但**仍可使用反应技能与看破**（不可主动普攻/技能/防御）；蓄力期间右侧面板切换为固定**【1.继续蓄力 2.终止蓄力】**，完成后变为**【1.释放蓄力 2.终止蓄力】**，释放时若满足选目标条件（敌方单体/带 `[他人]` 的我方单体）则进入目标选择，否则直接释放并触发其全部效果（含自身携带的 `[加速:N]`）。
- **`[加速:N]`**：永久加速库存（不自动过期，仅我方）。**只在蓄力结算完成时才施加**到角色身上；之后每次蓄力按"所需延迟回合"抵扣库存。
...
> 单文件引擎内联所有 JS，按职责划分为以下模块。行号基于 `战斗前端-爬塔 V5.2.html`（约 6925 行）。
