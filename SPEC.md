...
- **战术撤离判定（V6.12）**：
  - 撤离已右上角独立，改为行动面板第四个按钮【💨 尝试撤离】（仅我方回合可用）。
  - 实时均速：我方均速 $= \text{avg}_{我}(基础 Speed + 速度buff求和)$（复用 `getEffectiveSpeed`，含 `[单速]/[群速]/[单缓]/[群缓]` 临时增益/减益），敌方同理。
  - 对抗 roll（同命中/闪避公式方向）：$Roll_{我} = \text{Random}(1 \sim 我方均速)$，$Roll_{敌} = \text{Random}(1 \sim 敌方均速)$，**$Roll_{我} > Roll_{敌}$ 才成功**。
  - 成功：写成功文案 → 弹战后结算面板（含发送到酒馆按钮）。失败：写失败文案 → 消耗该行动角色本回合（`endTurn`），不弹面板，战斗继续。两结果均写入战局记录。
  - 右上角【记录】：仅只读展示战局历史，不写撤离文案、不发送到酒馆。
...
- **`[肃正]` / `[肃正:N]`**：为我方小队生成全队共享的“伪实体肉盾”圣域结界屏障（`teamBarrier`）。屏障数值与各英雄个人护盾（`entity.shield`）完全独立，开局清零。每次施放以本次技能 `power × 倍率` 为固定耐久上限（`teamBarrierMax = teamBarrier = actualPower`），屏障破碎时耐久上限与护甲/克制属性随之清零重置。
- **屏障存在时灰卡禁用**：当 `teamBarrier > 0` 且技能包含【肃正】标签时，技能面板自动灰卡禁用（`isEnough` 链路），点击无响应且拦截蓄力入口，屏障击破后方可再次施放。
- **克制属性动态主题配色**：根据克制属性（法术=gold / 远程=blue / 无属性或近战=white）动态挂载 `data-theme` 属性，由 CSS 变量组统一驱动网格、光晕、边框及徽章配色。
...
- **底层关联函数**：`applyBarrierHit()`、`getTauntTarget()`、`executeSkillAction()`、`TAG_HANDLERS['肃正']`、`updateTeamBarrierUI()`、`getBarrierTheme()`、`skillHasBarrierTag()`、`applyBarrierResist()`（屏障护甲/克制统一折算）、`parseSkill()`（`[肃正:子类型;power:N;Armor:N]` 解析）。
...
### 4.2.6 `[单速]` / `[群速]` / `[单缓]` / `[群缓]` 速度增减标签（V6.11）

- **`[单速;power:x]` / `[群速;power:x]`**：提升目标行动速度（加速）；**`[单缓;power:x]` / `[群缓;power:x]`**：降低目标行动速度（减速）。
- **独立临时速度 buff**：Buff 结构 `{ type:'spd', value, duration }`（加速正 / 减速负）。**不写回角色基础 `spd`、不改变闪避上限**（基础闪避值仍取自原始 Speed），仅作用于行动顺序排轴。
- **有效速度** = 基础 `spd` + 该角色身上所有 `type:'spd'` buff 求和，由 `getEffectiveSpeed()` 计算。
- **触发时机**：
  - **当前回合实时重排**：施放瞬间 `reorderRemainingQueue()` 对"剩余队列"（`queueIndex` 之后）按最新有效速度整体重排——保留已完成前缀（含施法者），**施法者不因加速额外获得一次行动**（除非技能带 `[再动]`）。
  - **下一回合持续生效**：`startRound()` 排轴改用 `getEffectiveSpeed()`（原直接用 `spd`），速度增减在下一回合起仍反映到排轴；buff 持续回合走 `nextTurn()` 通用 `duration--` 递减。
  - **多动单位（`actCount>1`）**：startRound 排的多条行动带 `queueSpd`，参与速度排序并按有效速度衰减（`有效速度 - 序数×10`）——减速后其后续行动会延后，不再固定霸占队首；而 `[再动]`/多重施法 splice 插入的条目（无 `queueSpd`）仍按最高优先级紧插当前之后不被冲散。
- **目标取向与命中**：`[单速]` 属纯增益单体（带 `[他人]` 可对我方指定目标）、`[群速]` 对我方全体（`isBeneficial` 含 `'速'` 归我方）；`[单缓]` 对敌方单体、`[群缓]` 对敌方全体，`缓` 纳入 `isDebuffTag` 作为敌方减益随命中判定（带攻击标签时一次命中即施加全部妨害，纯减速走完整闪避判定），接 `[必中]` 时强制命中。`classifySkill().hasSingleBuff` 含 `单速`、`hasEnemyTargetTag` 含 `缓`。
- **UI**：`updateBuffUI()` 新增速度徽章（`⚡速+N·M回` 青 / `🐌速-N·M回` 紫）；`formatTagPill` 为 `'速'`（cyan）/`'缓'`（purple）配色。
- **底层关联函数**：`TAG_HANDLERS['速']` / `TAG_HANDLERS['缓']` (`index.html:4862/4871`)、`getEffectiveSpeed()` (`index.html:4881`)、`reorderRemainingQueue()` (`index.html:4888`)、`startRound()` 排轴 (`index.html:6653`)。

### 4.3 目标取向与 `[他人]` 标签标准

- **目标取向判定（全标签扫描）**：技能的目标取向由 `classifySkill()` 扫描 `type/type2/type3` 全部标签一次定好，**顺序无关**：
  - 含任一敌方单体伤害/妨害标签（`单体/穿透/眩晕/单降/单盲/单滞/单弱/单缓`）→ 进入**敌方选目标**（即使第一个标签是群攻）。
  - 含 `[他人]` 且含任一纯有益单体标签（`单回/单增/单防/单盾/单瞄/单冲/单速/嘲/避/免伤/反击/驱散`）→ 进入**我方选目标**（不排除自己）。
